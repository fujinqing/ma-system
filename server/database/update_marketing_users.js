...
async function updateMarketingUsers() {
  const pool = await getPool();

  try {
    console.log('Updating Marketing department users...');

    // 查找市场&海外营销部的ID
    const deptResult = await pool.request()
      .query("SELECT id FROM sys_departments WHERE name LIKE '%市场%' OR name LIKE '%营销%'");
    
    console.log('Marketing departments:', deptResult.recordset);
    ...
    const marketingDeptId = deptResult.recordset[0].id;
    console.log('Marketing department ID:', marketingDeptId);

    // 更新或插入人员数据
    const users = [
      { name: '方豪', position: '市场 & 海外营销经理', isLeader: true },
      { name: '冯丽萍', position: '市场 & 海外营销部主管' },
      { name: '束欣润', position: '市场专员' },
      { name: '纪冉', position: '市场专员' }
    ];
    ...
    for (const user of users) {
      ...
      if (userResult.recordset.length > 0) {
        ...
        await pool.request()
          ...
          .input('department_id', sql.Int, marketingDeptId)
          .query(`
            UPDATE sys_users 
    ...
    const verifyResult = await pool.request()
      .input('dept_id', sql.Int, marketingDeptId)
      .query(`
